        ...
        for (int i = 0; i < perlinGrid.GetLength(0); i++)
        {
            for (int j = 0; j < perlinGrid.GetLength(1); j++)
            {
                for (int k = 0; k < perlinGrid.GetLength(2); k++)
                {
                    ...
                    //Debug.Log("Perlin grid x: " + perlinGrid[i, j, k].x);
                    /*if (perlinGrid[i, j, k].x < 0 || x < perlinGrid[i, j, k].x)
                    {
                        perlinGrid[i, j, k] = Vector3.zero;
                        continue;
                    }

                    if (perlinGrid[i, j, k].y < 0 || y < perlinGrid[i, j, k].y)
                    {
                        perlinGrid[i, j, k] = Vector3.zero;
                        continue;
                    }

                    if (perlinGrid[i, j, k].z < 0 || z < perlinGrid[i, j, k].z)
                    {
                        perlinGrid[i, j, k] = Vector3.zero;
                        continue;
                    }*/
        ...
        for (int i = 0; i < x; i++)
        {
            for (int j = 0; j < y; j++)
            {
                for (int k = 0; k < z; k++)
                {
                    ...
                    if (perlinGrid[i, j, k].Equals(Vector3.zero)) continue;
        ...
        for (int i = 0; i < x; i++)
        {
            for (int j = 0; j < y; j++)
            {
                for (int k = 0; k < z; k++)
                {
                    ...
                    perlinGrid[i, j, k].y = normalizePerlinNoise(perlinGrid[i, j, k].y, minNoise, (maxNoise));
